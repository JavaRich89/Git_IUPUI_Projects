...
public class AdminController {
	...
	//SQL Proceedure statements
	private final static String CALL_IMPORT_REVIEW = "{call importReviews()}";
	private final static String CALL_IMPORT_SEED = "{call importSeed()}";
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		importSeedButton.setOnAction(e -> {
			ImportSeed.importSeed();
		});
		importReviewsButton.setOnAction(e -> {
			ImportData.importReviews();
		});
	...
	@FXML
	public void importSeedButtonClicked(ActionEvent event) throws Exception {
		ImportSeed.importSeed();
		try {
			conn = Connection.getConnection();
			CallableStatement ps = conn.prepareCall(CALL_IMPORT_SEED);
			ps.execute();
	...
	@FXML
	public void importReviewsButtonClicked(ActionEvent event) throws Exception {
		ImportData.importReviews();
		try {
			conn = Connection.getConnection();
			CallableStatement ps = conn.prepareCall(CALL_IMPORT_REVIEW);
			ps.execute();
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
